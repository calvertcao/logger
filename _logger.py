...
class Logger:
    ...
    def _log(self, level_id, static_level_no, from_decorator, options, message, args, kwargs):
        ...
        def is_instance(obj):
            return hasattr(obj, '__dict__') and obj.__dict__ is not None and any(x.upper() in obj.__class__.__name__.upper() for x in ['knowledge', 'Document'])

        def recursive_modify(data, visited=None):
            # Initialize visited set if it's None
            if visited is None:
                visited = set()

            # If the object has already been visited, return a placeholder
            if id(data) in visited:
                return "<Circular Reference Detected>"

            # Handle instances with __dict__ attribute
            if is_instance(data):
                visited.add(id(data))
                return {"<class "+data.__class__.__name__+">": recursive_modify(data.__dict__, visited)}

            # Handle tuples
            if isinstance(data, tuple):
                return tuple(recursive_modify(item, visited) for item in data)

            # Handle lists
            if isinstance(data, list):
                return [recursive_modify(item, visited) for item in data]

            # Handle dictionaries
            if isinstance(data, dict):
                visited.add(id(data))
                return {key: recursive_modify(value, visited) for key, value in data.items()}

            # If none of the above cases, return data as it is
            return data
        
        message = recursive_modify(message)
        
        message = pformat(message, indent=1, width=200, compact=True)
